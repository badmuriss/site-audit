...
The audit ships with examples drawn from one specific stack (Cloudflare Workers + React 19 + Vite + TanStack Query + better-auth + Radix UI). Those examples are illustrative, the discipline is the same across stacks.

This file is the adaptation guide for every other stack. Read it when the defaults in the main workflow do not match the project in front of you: you cannot find the URL, you cannot log in headless, or you need seed data the project does not ship.

## Path conventions — fallback chain

The audit reads existing directories first and only creates one if nothing matches.
...
- **Personas**: `docs/personas/` → `personas/` → `.audit/personas/`
- **Audit config (allowlist)**: `audit-config.yml` → `.audit/config.yml`
- **Audit reports**: `docs/audits/site-audit-YYYY-MM-DD.md` → `audits/<date>.md`

The audit reads project conventions before defaulting. If the project's agent instructions file (`AGENTS.md`, or `CLAUDE.md` on Claude Code) says *"audit reports go in `quality-reports/`"*, use that.
...
1. Check the project's agent instructions file (`AGENTS.md` / `CLAUDE.md`) or README for "URL" or "dev server"
