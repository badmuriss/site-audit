...
The discipline applies symmetrically to FAIL rows — they get reproduction + evidence + suspected location (the standard findings format). See "Every finding must include" in [SKILL.md](../SKILL.md) for the required fields.
...
The full AI-tell catalogue is design territory, and this audit only measures. It lives in the [incredibly-pretty-websites](https://github.com/badmuriss/incredibly-pretty-websites) skill, which owns every design decision this checklist does not gate.
...
- **Screenshot** at `docs/audits/<date>/<surface>-<state>.png` (or whatever path [project-adaptation.md](project-adaptation.md) resolves for this project)
